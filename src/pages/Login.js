...
const Login = () => {
  ...
  const handleLogin = (val) => {
    const payload = {
      type: selectedRole,
      phone: selectedRole !== "company" && val.phone ? `92${val.phone}` : undefined,
  ...
  return (
    <div className="flex justify-center items-center min-h-screen min-w-full bg-gradient ">
      <div className="bg-white xs:w-[90%] sm:w-[80%] md:w-[50%]  rounded-2xl p-5">
        ...
        {selectedRole && !otpFlag && (
          <EnterPhoneEmailForm onSubmit={handleLogin} loader={loader} role={selectedRole}/>
